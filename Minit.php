 ...
 *  @author   Seong Jae Lee <seongjae@gmail.com>
 *  @license  GPL-2.0+
 */
...
class MinitTemplate extends BaseTemplate {
	...
	/**
	 * @var SkinMinit  SkinMinit object we talk to
	 */
	public $skin;
...
<?php } ?><?php if ( $this->data['showjumplinks'] ) { ?>
		<div id="jump-to-nav"><?php $this->msg( 'jumpto' ) ?> <a href="#column-one"><?php
...
<?php foreach ( $this->getFooterLinks() as $category => $links ) { ?>
	...
	<?php foreach ( $links as $link ) { ?>
		<li id="footer-<?php echo $category ?>-<?php echo $link ?>"><?php
			$this->html( $link )
		?></li><?php
} ?>
	</ul><?php
...
<?php
	...
	protected function renderPortals( $sidebar ) {
		...
		if ( !isset( $sidebar['VIEW'] ) ) {
			$sidebar['VIEW'] = true;
		}
		if ( !isset( $sidebar['PERSONAL'] ) ) {
			$sidebar['PERSONAL'] = true;
		}
		if ( !isset( $sidebar['RC'] ) ) {
			$sidebar['RC'] = true;
		}
...
<?php
		...
		$rows = $dbr->select(
			$tables, '*', $conds, __METHOD__, array(
				'ORDER BY' => 'rc_timestamp DESC', 'LIMIT' => 50 ) + $query_options,
				$join_conds
		);
		$history = array();
		foreach ( $rows as $row ) {
